...
function createModel(opt)
	...
	local function ConvBNReLU(nInputPlane, nOutputPlane)
	  ...
	  vgg:add(backend.VolumetricConvolution(nInputPlane, nOutputPlane, 3,3,3, 1,1,1, 1,1,1))
	...
	local MaxPooling = backend.VolumetricMaxPooling
	
	--ConvBNReLU(1,64):add(nn.Dropout(0.3))
	ConvBNReLU(1,64):add(nn.VolumetricDropout(0.3))
	ConvBNReLU(64,64)
	vgg:add(MaxPooling(2,2,2, 2,2,2):ceil())
	
	ConvBNReLU(64,128):add(nn.VolumetricDropout(0.4))
	ConvBNReLU(128,128)
	vgg:add(MaxPooling(2,2,2, 2,2,2):ceil())
	
	ConvBNReLU(128,256):add(nn.VolumetricDropout(0.4))
	ConvBNReLU(256,256):add(nn.VolumetricDropout(0.4))
	ConvBNReLU(256,256)
	vgg:add(MaxPooling(2,2,2, 2,2,2):ceil())
	
	ConvBNReLU(256,512):add(nn.VolumetricDropout(0.4))
	ConvBNReLU(512,512):add(nn.VolumetricDropout(0.4))
	ConvBNReLU(512,512)
	vgg:add(MaxPooling(2,2,2, 2,2,2):ceil())
	...
	local a = vgg:cuda():forward(torch.randn(1,1,32,32,32):cuda())
	print(a:size())
