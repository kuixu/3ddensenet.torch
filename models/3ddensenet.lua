...
local function createModel(opt)
   ...
   function addTransition(model, nChannels, nOutChannels, opt, last, pool_size)
      ...
      model:add(cudnn.VolumetricBatchNormalization(nChannels))
      model:add(cudnn.ReLU(true))      
      if last then
         model:add(cudnn.VolumetricAveragePooling(pool_size, pool_size, pool_size))
         model:add(nn.Reshape(nChannels))      
      else
         model:add(cudnn.VolumetricConvolution(nChannels, nOutChannels, 1, 1, 1,  1, 1, 1,  0, 0, 0))
         if opt.dropRate > 0 then model:add(nn.Dropout(opt.dropRate)) end
         model:add(cudnn.VolumetricAveragePooling(2, 2, 2))
      end      
   end
   ...
   elseif opt.dataset == '3dimage0' then

      --number of layers in each block
      if opt.depth == 50 then
         stages = {6, 12, 24, 16}
      elseif opt.depth == 121 then
         stages = {6, 12, 24, 16}
      elseif opt.depth == 169 then
         stages = {6, 12, 32, 32}
      elseif opt.depth == 201 then
      ...
      --Initial transforms follow ResNet(32x32x32)
      model:add(cudnn.VolumetricConvolution(1, nChannels, 3,3,3, 2,2,2, 1,1,1))
      model:add(cudnn.VolumetricBatchNormalization(nChannels))
      model:add(cudnn.ReLU(true))
      model:add(nn.VolumetricMaxPooling(3, 3, 3,  2, 2, 2, 1, 1, 1))

      --Dense-Block 1 and transition (16x16x16)
      nChannels = addDenseBlock(model, nChannels, opt, stages[1])
      addTransition(model, nChannels, math.floor(nChannels*reduction), opt)
      nChannels = math.floor(nChannels*reduction)

      --Dense-Block 2 and transition (8x8x8)
      nChannels = addDenseBlock(model, nChannels, opt, stages[2])
      addTransition(model, nChannels, math.floor(nChannels*reduction), opt)
      nChannels = math.floor(nChannels*reduction)

      --Dense-Block 3 and transition (4x4x4)
      nChannels = addDenseBlock(model, nChannels, opt, stages[3])
      addTransition(model, nChannels, math.floor(nChannels*reduction), opt)
      --addTransition(model, nChannels, math.floor(nChannels*reduction), opt, true, 2)
      nChannels = math.floor(nChannels*reduction)

      --Dense-Block 4 and transition (2x2x2)
      nChannels = addDenseBlock(model, nChannels, opt, stages[4])
      addTransition(model, nChannels, nChannels, opt, true, 2)
   
   elseif opt.dataset == 'pdb' then
      --Initial convolution layer
      model:add(cudnn.VolumetricConvolution(1, nChannels, 3,3,3, 1,1,1, 2,2,2))
   ...
   ConvInit('cudnn.VolumetricConvolution')
   ...
   print(model)
   -- a=model:forward(torch.randn(1,1,16,16,16):cuda())
   -- a=model:forward(torch.randn(1,1,32,32,32):cuda())
   -- for modelnet40 
   w = 32
   a=model:forward(torch.randn(128,1,w ,w ,w):cuda())
   print(a:size())
   return model
end
