...
function DenseConnectLayerStandard(nChannels, opt)
   ...
   net:add(ShareGradInput(cudnn.VolumetricBatchNormalization(nChannels), 'first'))
   net:add(cudnn.ReLU(true))   
   if opt.bottleneck then
      net:add(cudnn.VolumetricConvolution(nChannels, 4 * opt.growthRate, 1, 1, 1,  1, 1, 1,  0, 0, 0))
      nChannels = 4 * opt.growthRate
      if opt.dropRate > 0 then net:add(nn.Dropout(opt.dropRate)) end
      net:add(cudnn.VolumetricBatchNormalization(nChannels))
      net:add(cudnn.ReLU(true))      
   end
   net:add(cudnn.VolumetricConvolution(nChannels, opt.growthRate, 3, 3, 3,  1, 1, 1,  1, 1, 1))
   if opt.dropRate > 0 then net:add(nn.Dropout(opt.dropRate)) end
...
function DenseConnectLayerCustom:__init(nChannels, opt)
   ...
   self.net1:add(ShareGradInput(cudnn.VolumetricBatchNormalization(nChannels), 'first'))
   self.net1:add(cudnn.ReLU(true))  

   self.net2 = nn.Sequential()
   if opt.bottleneck then
      self.net2:add(cudnn.VolumetricConvolution(nChannels, 4*opt.growthRate, 1, 1, 1,  1, 1, 1,  0, 0, 0))
      nChannels = 4 * opt.growthRate
      self.net2:add(cudnn.VolumetricBatchNormalization(nChannels))
      self.net2:add(cudnn.ReLU(true))
   end
   self.net2:add(cudnn.VolumetricConvolution(nChannels, opt.growthRate, 3, 3, 3,  1, 1, 1, 1, 1, 1))
