...
local M = {}
local Image3DDataset = torch.class('deconv.Image3DDataset', M)

function Image3DDataset:__init(imageInfo, opt, split)
   ...
   elseif opt.dataprepro == '4' then
	  ...
	  if torch.sum(image:gt(0))> 0 then 
...
function Image3DDataset:preprocess()
